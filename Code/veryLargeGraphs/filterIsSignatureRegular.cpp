...
int main()
{
    ...
    while(getline(cin,line))
    {
        ...
        int lambda=-2;
        int girth=-2;
        vector<int> modelSignature;
        bool isSignatureRegular=true;
        for(int u=0; u<n && isSignatureRegular; u++)
        {
            ...
            for(int v : graph[u])
            {
               ...
               signature.push_back(newNumPaths);
            }
            sort(signature.begin(),signature.end());
            if(modelSignature.size()==0) modelSignature=signature;
            bool same=true;
            for(int i=0; i<modelSignature.size() && same; i++)
            {
                if(modelSignature[i]!=signature[i]) same=false;
            }
            if(!same) isSignatureRegular=false;
        }
        if(isSignatureRegular) printf("%s\n",line.c_str());
    }
    return 0;
}
